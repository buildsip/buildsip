...
> Read `*.jsonl` files inside the returned `tempLogsDir`. Write one Markdown file per story inside the returned `tempDir`. Use filename slugs based on each title, such as `title-of-story-1.md`, `title-of-story-2.md`, and so on. Follow the `STORY.md` instructions. If nothing clears the **substance bar**, skip to the cleanup step. Here are the paths to the folders mentioned prior: ...
...
- If they request changes, make them now, at this step. Follow the rules in `STORY.md`, but if they conflict with the user's request, prioritize the user.
